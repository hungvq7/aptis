...
const bodySchema = z.object({ questionId: z.string(), bookmarked: z.boolean() })
...
export async function PATCH(request: Request, { params }: { params: Promise<{ attemptId: string }> }) {
  ...
  await repository.setBookmark(attemptId, parsed.data.questionId, parsed.data.bookmarked)
